...
st.set_page_config(page_title="Layer 2 Categorical Analysis", layout="wide")
st.title("Layer 2: Categorical Analysis")
st.info("Compare how well applications performed in specific categories based on verified human evaluations.")
...
if not db_rows or not req_list:
    st.warning("Ensure you have evaluated platforms in Layer 2 and Layer_2_Requirements.xlsx is present.")
    st.stop()
...
for req in req_list:
    ...
    if cat:
        # Each requirement is worth a max of 2 points
...
for name, score_json in db_rows:
    try:
        scores_dict = json.loads(score_json) if score_json else {}
        # Extract the score from the nested dict: {"ReqID": {"score": 2, "evaluator": "Jane"}}
        total_score = sum(details.get("score", 0) for details in scores_dict.values() if isinstance(details, dict))
    except Exception:
        total_score = 0
...
# SELECT APPS 
st.sidebar.header("Comparison Settings (Layer 2)")
...
def get_cat_performance(app_name):
    ...
    try:
        scores_dict = json.loads(app_data[1]) if app_data[1] else {}
    except Exception:
        scores_dict = {}
    ...
    for req in req_list:
        ...
        if cat and rid:
            ...
            score = scores_dict.get(rid, {}).get("score", 0)
...
if len(selected_apps) == 1:
    ...
    # Build comparison data for 2 or 3 apps
...
st.divider()
st.caption("Note: Percentages represent the platform's competency relative to the maximum possible points in each category, based on Layer 2 human evaluations.")